...
namespace UCDMvcBootCamp.Helpers
{
    ...
    public class DummyDataLoader : IDummyDataLoader
    {
        ...
        public DummyDataLoader(IRepository repository)
        {
        ...
        public void CreateDb()
        {
            var configuration = Fluently
                .Configure()
                .Mappings(x => x.FluentMappings.AddFromAssembly(typeof(Conference).Assembly)
                       .Conventions.AddAssembly(typeof(PrimaryKeyConvention).Assembly))
                .BuildConfiguration();

            var sqlLiteDialect = new NHibernate.Dialect.SQLiteDialect();
            var dropScripts = configuration.GenerateDropSchemaScript(sqlLiteDialect);
            var creationScript = configuration.GenerateSchemaCreationScript(sqlLiteDialect);

            ExecuteScripts(dropScripts);
            ExecuteScripts(creationScript);
        }

        private void ExecuteScripts(IEnumerable<string> scripts)
        {
        ...
        public void Load()
        {
            using (var tx = _session.BeginTransaction())
            {
                ...
                _session.SaveOrUpdate(codeMash);
                _session.SaveOrUpdate(mix10);
                _session.SaveOrUpdate(pdc);
        ...
        private static void GenerateAttendees(Conference @event)
        {
